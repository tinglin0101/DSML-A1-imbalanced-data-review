...
def oversampling_keel_data(filepath, output_filepath):
    """
    讀取 KEEL 格式資料，對特徵進行 Oversampling，再輸出為相同格式
    """
    # 讀取原始檔案
...
def main(base_path=r".\dataset",dataset_name="yeast1-5-fold"):
    ...
    output_dir = os.path.join(base_path, dataset_name + "_oversampling")
    ...
    for filename in os.listdir(input_dir):
        ...
        if filename.endswith('tra.dat'): 
            ...
            try:
                oversampling_keel_data(input_path, output_path)
