...
def borderline_smote_keel_data(filepath, output_filepath):
    """
    讀取 KEEL 格式資料，對特徵進行 Borderline-SMOTE，再輸出為相同格式
    """
    # 讀取原始檔案
...
def main(base_path=r".\dataset",dataset_name="yeast1-5-fold"):
    ...
    output_dir = os.path.join(base_path, dataset_name + "_borderline_smote")

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        
    # print(f"開始針對 {input_dir} 中的訓練集執行 Borderline-SMOTE 過採樣...")
    ...
    for filename in os.listdir(input_dir):
        ...
        if filename.endswith('tra.dat'): 
            ...
            try:
                borderline_smote_keel_data(input_path, output_path)
